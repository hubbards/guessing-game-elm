...
update msg model =
    case msg of
        ...
        Correct ->
            ( { model | guesses = Tree.Empty }, Cmd.none )

        Lower ->
            case model.guesses of
                Tree.Node _ l _ ->
                    ( { model | guesses = l }, Cmd.none )
        ...
        game =
            case model.guesses of
                ...
                Tree.Node n Tree.Empty Tree.Empty ->
                    [ Html.text <| "Your number is " ++ String.fromInt n ]

                Tree.Node n _ _ ->
                    [ Html.text <| "Is your number " ++ String.fromInt n ++ "?"
                    , Html.button [ Html.Events.onClick Correct ] [ Html.text "Correct" ]
                    , Html.button [ Html.Events.onClick Lower ] [ Html.text "Lower" ]
                    , Html.button [ Html.Events.onClick Higher ] [ Html.text "Higher" ]
                    ]
    in
    Html.div
        []
        [ Html.p
            ...
            , Html.br [] []
            , Html.text (Debug.toString model.choices)
            ]
        , Html.button
